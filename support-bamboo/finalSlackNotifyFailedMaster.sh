...
if [ "${bamboo_inject_jobResult}" != "Success" ]; then
    ...
    commitCount=`curl https://codice.atlassian.net/builds/rest/api/latest/result/${bamboo_planKey}-${bamboo_buildNumber}  | sed 's/.*changes size=\"//' | sed 's/\".*//'`
    ...
    if [ ${commitCount} == "0" ]; then
        ...
        parent=`curl https://codice.atlassian.net/builds/rest/api/latest/result/${lastParent} | sed 's/.*\<buildReason//' | sed 's/.*&quot;&gt;//' | sed 's/&lt;.*//'`
        ...
        commitCount=`curl https://codice.atlassian.net/builds/rest/api/latest/result/${lastParent}  | sed 's/.*changes size=\"//' | sed 's/\".*//'`
    fi

    # If still no commits, blame manual run
    if [ ${commitCount} == "0"  ]; then
        curl -X POST --data-urlencode "payload={\"text\": \":warning: *Build Failure on Master* - <${bamboo_resultsUrl}|${bamboo_planName} - #${bamboo_buildNumber}>\n\`\`\`Manual Build by ${parent}\`\`\`\", \"channel\": \"$1\", \"username\": \"bamboo-bot\", \"icon_emoji\": \":bamboo:\"}" $2

   # If commits, display offending commits
    else
        commitMessages=`git log --pretty=format:'%h %an %s' -n ${commitCount}`
        curl -X POST --data-urlencode "payload={\"text\": \":warning: *Build Failure on Master* - <${bamboo_resultsUrl}|${bamboo_planName} - #${bamboo_buildNumber}> - Commits:\n\`\`\`${commitMessages}\`\`\`\", \"channel\": \"$1\", \"username\": \"bamboo-bot\", \"icon_emoji\": \":bamboo:\"}" $2
