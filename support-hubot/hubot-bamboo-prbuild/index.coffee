        ...
        repoDeltas = (base_url, pr_num, callback) ->
          robot.http(base_url + "/pulls/" + pr_num + "/files")
            ...
            .get() (err, res, body) ->
              ...
              redFunc = (pv, cv) ->
                if cv.length > 1 and cv[0] not in pv and cv[0] != "distribution"
                  pv.concat(cv[0])
                else
                  pv
              ...
              if root_delta or directories.length > 1
                callback(root_pr_plan, "IGNORE")
              else if directories.length == 0
                callback(distro_pr_plan, "IGNORE")
              else
