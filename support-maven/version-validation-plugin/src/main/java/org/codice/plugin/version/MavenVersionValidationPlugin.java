...
public class MavenVersionValidationPlugin extends AbstractMojo {
  ...
  private Set<Character> rangeSymbols = new HashSet<>(
      Arrays.asList('>', '=', '|', '-', '<', '^', '~'));
  ...
  private static final String MOJO_EXCEPTION_MESSAGE =
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    if (baseDir != null) {
      String[] fileList = getListOfPackageJsonFiles(baseDir);

      for (String filepath : fileList) {
        File jsonFile = Paths.get(baseDir, File.separator, filepath)
            .toFile();

        try {
          String jsonContents = FileUtils.readFileToString(jsonFile, (Charset) null);
          try (JsonReader jsonReader = Json.createReader(new StringReader(jsonContents))) {
   ...
   */
  private String[] getListOfPackageJsonFiles(String baseDir) {

    DirectoryScanner scanner = new DirectoryScanner();

    scanner.setIncludes(new String[]{"**/*" + MavenVersionValidationPlugin.FILE_NAME});
  ...
  void setWhitelistedValues(String[] whitelistedValues) {
    this.whitelistedValues = whitelistedValues;
  }

  private String[] getWhitelistedValues() {
