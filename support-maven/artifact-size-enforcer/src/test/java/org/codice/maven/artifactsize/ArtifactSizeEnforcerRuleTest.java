...
import static org.codice.maven.artifactsize.ArtifactSizeEnforcerRule.PROJECT_ARTIFACT_ID_PROP;
import static org.codice.maven.artifactsize.ArtifactSizeEnforcerRule.PROJECT_BUILD_DIR_PROP;
import static org.codice.maven.artifactsize.ArtifactSizeEnforcerRule.PROJECT_PACKAGING_PROP;
import static org.codice.maven.artifactsize.ArtifactSizeEnforcerRule.PROJECT_VERSION_PROP;
import static org.codice.maven.artifactsize.ArtifactSizeEnforcerRule.SUPPORTED_PACKAGE_TYPES;
...
public class ArtifactSizeEnforcerRuleTest {
  ...
  @Test
  public void respectSkipFlag() throws EnforcerRuleException, IOException {
    String sampleArtifactPath = tempFolder.newFile("SampleArtifact.jar").getPath();
    ArtifactSizeEnforcerRule enforcer = new ArtifactSizeEnforcerRule()
        .setArtifactLocation(sampleArtifactPath);

    MockEnforcementRuleHelper ruleHelper = new MockEnforcementRuleHelperNoEvaluateCall()
        .artifactId(SAMPLE_ARTIFACT_ID)
        .packaging(ArtifactSizeEnforcerRule.JAR)
        .version(SAMPLE_VERSION)
        .buildDir(sampleBuildDir.getPath());
    enforcer.setSkip(true);

    enforcer.execute(ruleHelper);
  }
  ...
  public void testByteConversion() throws EnforcerRuleException {
    ...
    assertEquals(enforcer.maxArtifactSizeToBytes(defaultMockhelper), 1024L * 1024);
  }

  @Test(expected = EnforcerRuleException.class)
  public void invalidByteConversion() throws EnforcerRuleException {
    ArtifactSizeEnforcerRule enforcer = new ArtifactSizeEnforcerRule();
    enforcer.setMaxArtifactSize("1024.3_B");
    assertEquals(enforcer.maxArtifactSizeToBytes(defaultMockhelper), 1024);
  }

  @Test(expected = EnforcerRuleException.class)
  public void invalidKilobyteConversion() throws EnforcerRuleException {
    ArtifactSizeEnforcerRule enforcer = new ArtifactSizeEnforcerRule();
    enforcer.setMaxArtifactSize("2xyz_KB");
    assertEquals(enforcer.maxArtifactSizeToBytes(defaultMockhelper), 1024);
  }

  @Test(expected = EnforcerRuleException.class)
  public void invalidMegabyteConversion() throws EnforcerRuleException {
    ArtifactSizeEnforcerRule enforcer = new ArtifactSizeEnforcerRule();
    enforcer.setMaxArtifactSize("1abc_MB");
    assertEquals(enforcer.maxArtifactSizeToBytes(defaultMockhelper), 1024);
  }

  static class MockEnforcementRuleHelper implements EnforcerRuleHelper {
  ...
  static class MockEnforcementRuleHelperNoEvaluateCall extends MockEnforcementRuleHelper {

    @Override
    public Object evaluate(String s) throws ExpressionEvaluationException {
      throw new TestingException("Should not be invoked");
    }
  }
